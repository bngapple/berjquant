...
def fitness_maximize(metrics, mc, trades):
    ...
    dd_penalty = max(0, (dd - 1800) * 10) if dd > 1800 else 0
...
def mutate_strategy(sd, intensity=0.3):
    ...
    if sz["method"] == "fixed":
        ...
        sz["fixed_contracts"] = min(15, new_ct)
...
""")
  ...
  ══════════════════════════════════════════════════════════════""")
    ...
    if population:
        ...
        with open("reports/maximized_strategies.json", "w") as f:
            json.dump(output, f, indent=2, default=str)
        print(f"\n  Saved to reports/maximized_strategies.json")
