#!/usr/bin/env python3
"""
Export every trade from HTF Swing Hybrid v2 LucidFlex to CSV.

Three CSVs: all_trades, monthly_summary, daily_summary.

Usage:
    python3 run_htf_swing_export_v2.py
"""
...
SEED = 42
np.random.seed(SEED)
REPORTS = Path("ANTHONY")
_ET = ZoneInfo("US/Eastern")

C = 3  # contracts per strategy
STRATS = {
    "RSI": {"sig": lambda d: sig_rsi_extreme(d, 5, 65, 35), "sl": 40, "tp": 400, "hold": 5},
    "IB":  {"sig": lambda d: sig_ib_breakout(d)[0],          "sl": 40, "tp": 480, "hold": 15},
...
def main():
    t0 = _time.time()
    print("═" * 70)
    print("  HTF SWING HYBRID v2 — COMPLETE TRADE EXPORT")
    print("═" * 70)
    ...
    rsi7 = calc_rsi(closes, 5)  # Hybrid v2 uses RSI(5)
    ...
    # ── Write trades CSV ──
    trades_path = REPORTS / "htf_swing_v2_all_trades.csv"
    ...
    monthly_path = REPORTS / "htf_swing_v2_monthly_summary.csv"
    ...
    daily_path = REPORTS / "htf_swing_v2_daily_summary.csv"
